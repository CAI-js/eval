...
function walkObject(node, context) {
  ...
  for (let i = 0, l = node.properties.length; i < l; i += 1) {
    ...
    const value = walk(prop.value, context);
    if (value === failResult) {
      return failResult;
    }
...
function walkCall(node, context) {
  ...
  if (callee === failResult || typeof callee !== 'function') {
    return failResult;
  }
  let ctx = node.callee.object ? walk(node.callee.object, context) : failResult;
  if (ctx === failResult) {
    ctx = null;
  }
  const args = [];
...
function walkMember(node, context) {
  ...
  if (obj === failResult || typeof obj === 'function') {
    return failResult;
  }
...
function walkFunction(node, context) {
  ...
  for (let i = 0, l = bodies.length; i < l; i += 1) {
    if (walk(bodies[i], newContext) === failResult) {
      return failResult;
    }
  }
...
function evaluateAll(str, context = {}, allowChanges = true) {
  const newContext = allowChanges ? context : deepFreeze(context);
  const result = [];
  ...
  for (let i = 0; i < compiled.body.length; i += 1) {
    const expression = compiled.body[i].expression
      ...
      : compiled.body[i];
    const value = walk(expression, newContext);
    result.push(value === failResult ? undefined : value);
  }
  return result;
}

function evaluate(str, context = {}, allowChanges) {
  const result = evaluateAll(str, context, allowChanges);
